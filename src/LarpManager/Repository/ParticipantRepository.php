...
class ParticipantRepository extends EntityRepository
{
	...
	{
		$participants = $this->getEntityManager()
		->createQuery('SELECT p FROM LarpManager\Entities\Participant p JOIN p.user u JOIN u.etatCivil ec WHERE p.groupe IS NOT NULL AND ec.fedegn IS NOT NULL AND REGEXP(ec.fedegn, \'^[A-Za-z0-9]{5}\') = true ORDER BY ec.nom ASC, ec.prenom ASC')
		->getResult();
	...
	{
		$participants = $this->getEntityManager()
		->createQuery('SELECT p FROM LarpManager\Entities\Participant p JOIN p.user u JOIN u.etatCivil ec WHERE p.groupe IS NOT NULL AND (ec.fedegn IS NULL OR REGEXP(ec.fedegn, \'^[A-Za-z0-9]{5}\') = false) ORDER BY ec.nom ASC, ec.prenom ASC')
		->getResult();
